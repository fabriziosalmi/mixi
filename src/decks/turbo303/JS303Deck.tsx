...
// Mixi – JS303 Deck UI
//
// Layout: HEADER → SEQUENCER → SYNTH KNOBS → FX → TRANSPORT
// ─────────────────────────────────────────────────────────────
...
export const JS303Deck: FC<HouseDeckProps> = ({ deckId, color, onSwitchToTrack }) => {
  ...
  const bpm = snapshot.syncToMaster ? engine.bpm : snapshot.bpm;
    ...
    <div className="flex flex-col h-full w-full select-none overflow-hidden" style={{ fontSize: 11 }}>
      ...
      <div className="flex items-center justify-between px-3 py-1.5" style={{ borderBottom: '1px solid var(--brd-default)' }}>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              engine.syncToMaster = !snapshot.syncToMaster;
          ...
          <button onClick={onSwitchToTrack} className="text-[9px] text-zinc-500 hover:text-white px-1">EJECT</button>
        </div>
      </div>
        ...
        {snapshot.steps.map((step, i) => (
          ...
          >
            ...
            <div
              ...
              style={{ background: step.accent && step.gate ? '#f59e0b' : 'rgba(255,255,255,0.08)' }}
              title="Accent"
            />
            {/* Slide dot */}
            <div
              ...
              className="w-1.5 h-1.5 rounded-full cursor-pointer"
              style={{ background: step.slide && step.gate ? '#06b6d4' : 'rgba(255,255,255,0.08)' }}
              title="Slide"
            />
            {/* Note label */}
      ...
      <div className="flex items-center justify-center gap-3 px-3 py-3 flex-1">
        ...
        <button
          ...
          style={{ border: '1px solid var(--brd-default)' }}
        >
      ...
      <div className="flex items-center justify-between px-3 py-2" style={{ borderTop: '1px solid var(--brd-default)' }}>
        ...
        <div className="flex items-center gap-3">
          <div className="flex flex-col items-center gap-0.5">
            <Knob value={snapshot.swing} min={0} max={0.5} onChange={(v: number) => { engine.swing = v; setSnapshot(s => ({ ...s, swing: v })); }} color="var(--txt-muted)" scale={0.5} />
            <span className="text-[7px] text-zinc-600">SWG</span>
          </div>
          <div className="flex flex-col items-center gap-0.5">
            <Knob value={snapshot.masterVolume} min={0} max={1} onChange={(v: number) => { engine.masterVolume = v; setSnapshot(s => ({ ...s, masterVolume: v })); }} color="var(--txt-muted)" scale={0.5} />
            <span className="text-[7px] text-zinc-600">VOL</span>
          </div>
          <button onClick={() => { engine.clearPattern(); setSnapshot(s => ({ ...s, steps: s.steps.map(st => ({ ...st, gate: false })) })); }}
            className="text-[8px] font-bold text-zinc-500 hover:text-red-400 px-1.5 py-0.5 rounded border border-zinc-800 active:scale-95">
            CLR
          </button>
          <button onClick={() => { engine.resetPattern(); setSnapshot(s => ({ ...s, steps: defaultSteps() })); }}
            className="text-[8px] font-bold text-zinc-500 hover:text-white px-1.5 py-0.5 rounded border border-zinc-800 active:scale-95">
            ACID
          </button>
        </div>
      </div>
