/**
 * Mixi Native Output Tap — AudioWorklet Processor
 *
 * Captures audio samples from the WebAudio graph and writes them
 * into a SharedArrayBuffer (SPSC ring buffer) for consumption by
 * the native cpal audio stream.
 *
 * Ring buffer layout (matches mixi-native/src/lib.rs):
 *   [0..3]  = write_head (u32, atomic)
 *   [4..7]  = read_head  (u32, atomic)
 *   [8..]   = interleaved f32 samples (stereo: L, R per frame)
 */
class NativeOutputTapProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  process(inputs, outputs) {
    ...
    if (free < frames) {
      // Ring buffer full — drop samples (native side is too slow)
      return true;
    }
...
registerProcessor('native-output-tap', NativeOutputTapProcessor);
